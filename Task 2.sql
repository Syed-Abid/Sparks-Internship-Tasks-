...
SELECT DISTINCT(Year), COUNT(Year) AS Attack_years
FROM dbo.Terrorism
GROUP BY Year
ORDER BY Attack_years DESC;

-- Most Common attacktype? ( Use in Dashboard)
SELECT DISTINCT(Attacktype1),COUNT(Attacktype1) AS AttackMethods
FROM dbo.Terrorism
GROUP BY Attacktype1
ORDER BY AttackMethods DESC;
